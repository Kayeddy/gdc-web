...
// Validations imports
import { LeaderOnboardingValidation } from "@/lib/validations/zod/onboarding";
...
/**
 * Renders a form for the onboarding of leaders. This form includes various fields
 * necessary for collecting a leader's profile information and is designed to handle
 * dynamic inputs like university email extensions and social media links.
 *
 * @param {object} props - Component props.
 * @param {(userData: Iuser) => void} props.onSubmitHandler - Function to handle the form submission.
 * @param {string} props.selectedProfileType - The type of profile selected by the user.
 * @param {(profileType: string) => void} props.onProfileTypeChange - Function to handle changes to the profile type.
 * @param {boolean} props.isSubmitting - Indicates whether the form is currently submitting.
 * @returns {React.ReactElement} - A form component for leader onboarding.
 */
export default function LeaderOnboardingForm({
  onSubmitHandler,
  selectedProfileType,
  onProfileTypeChange,
  isSubmitting,
  ...
  const { control, handleSubmit, reset, setValue, getValues } = useForm<Iuser>({
    resolver: zodResolver(LeaderOnboardingValidation),
    defaultValues: {
            ...
            >
              ...
              <Controller
                ...
                render={({ field, fieldState: { error } }) => (
                  <RadioGroup
                    {...field}
                    label="¿Tienes experiencia liderando equipos?"
                    className="w-full"
                    isInvalid={error ? true : false}
                    isDisabled={isSubmitting}
                    errorMessage={error?.message}
              ...
              <Controller
                name="developedProjects"
                control={control}
                render={({ field, fieldState: { error } }) => (
                  <Textarea
                    {...field}
                    label="Proyectos"
                    placeholder="Cuéntanos sobre algunos de tus proyectos Tech más asombrosos!"
                    isInvalid={!!error}
                    isDisabled={isSubmitting}
                    disabled={isSubmitting}
                    errorMessage={error?.message}
